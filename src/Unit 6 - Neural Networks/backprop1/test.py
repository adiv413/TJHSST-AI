...
def main():
    x = [[1, 2, 3], [2, 6], [1], [1]]
    weights = [[[4, 2], [5, 3], [6, 4]], [[2], [3]], [[3]]]

    transfer_function = logistic
    transfer_function_dx = logistic_dx

    forward_prop(x, weights, transfer_function)
    print(x)
    backprop(x, weights, transfer_function_dx, [2], 0.1)
    print(weights)
...
def backprop(x_values, weights, transfer_function_dx, expected_outputs, alpha):
    ...
    gradients = [[[0.0 for k in j] for j in i] for i in weights]
    ...
    for layer in range(len(x_values) - 2, -1, -1): # layer
        for i in range(len(x_values[layer])): # each node in layer
            print()
            print("layer and i", layer, i)
            print("curr weigths", weights)
            print("curr errors", errors)
            print("x", x_values)
            if layer == len(x_values) - 2:
                ...
                weights[layer][i][0] += alpha * first_neg_gradient
                ...
                for j in range(len(errors[layer + 1])):
                    print("layer and i and j", layer, i, j)
                    ...
                    print("calculating erorr", weights[layer][i][j], errors[layer + 1][j])
                    print("added error", weights[layer][i][j] * errors[layer + 1][j])
                    neg_gradient = x_values[layer][i] * errors[layer + 1][j]
                    print("gradient", layer, i, j, neg_gradient)
                    weights[layer][i][j] += alpha * neg_gradient
