...
def main():
    ...
    if args and args[0]:
        if args[0].isdigit():
            ...
            if len(args[0]) == 2 and 'a' <= args[0][0].lower() <= 'h' and args[0][1].isdigit() and 0 <= int(args[0][1]) <= 8:
                move = (int(args[0][1]) - 1) * 8 + ord(args[0][0].lower()) - ord('a')

    oppositeToken = 'o' if tokenToMove == 'x' else 'x'

    # first snapshot

    initial_moves = find_or_make_moves(board, tokenToMove, oppositeToken)
    ...
    if initial_moves:
        print('Possible moves for', tokenToMove + ':', initial_moves)
    else:
